...
=======
	...
	"fmt" //TOREMOVE
>>>>>>> 0a7c909133c98fc41b07b8119814305b28c7bac9
)

//Constant values
const DEFAULTROLLOUT = 120 // WAS 300
...
=======
									...
									fmt.Println("Already existing", g.slice_results) //TOREMOVE
...
func (g *Gossiper) ReceiveDecisionExpelRequest(message RequestMessage, decision int) {
	//Once the decision has been taken we have the result..
	log.Lvl1("Decision for ", message.Recipient,", is :" , decision)
	var reply RequestReply
	if decision == 1 {
		reply = RequestReply{
			Banned:             true,
...
func (g *Gossiper) AnnounceNewMasterKey() error {
	...
	//fmt.Println("Authorities are now", auth)
